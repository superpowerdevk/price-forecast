...
3. **Print the card verbatim.** Everything above the `[AGENT INSTRUCTIONS]` line is the forecast card — normally a **self-contained inline SVG chart** (a dark forecast terminal: 90-day price history flowing into a shaded forecast cone, median path, spot line, and stat tiles for conviction / expected close / range / move odds, currency-correct). Output it **AS-IS as a raw inline HTML/SVG block** — not inside a code fence, not escaped, not redrawn or described, and don't add a separate text version of the numbers. The SVG *is* the card. (If the sidecar returns no chart data, the card falls back to a Markdown text block instead — the embedded instructions tell you which; follow them.) Then build the headline + read sections **below** the card.
